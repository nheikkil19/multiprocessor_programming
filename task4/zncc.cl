                    ...
                    if ( y >= 0 && y < h ) {
                        for (int win_x=0; win_x<win_size; win_x++) {
                            ...
                            if ( x >= 0 && x-(d*inv) >= 0 && x < w && x-(d*inv) < w ) {
                                zncc1 += (read_imageui(imageInL, (int2)(x, y)).x - imgAvgL) * (read_imageui(imageInR, (int2)(x-(d*inv), y)).x - imgAvgR);
                                zncc2 += pow(read_imageui(imageInL, (int2)(x, y) ).x  - imgAvgL, 2);
                                zncc3 += pow(read_imageui(imageInR, (int2)(x-(d*inv), y)).x - imgAvgR, 2);
                            }
                        }
                    }
                }
